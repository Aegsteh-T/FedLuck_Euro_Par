...
class ERAFLClient:
    ...
    def __getstate__(self):
        ...
        res_keys = ['cid', 'W', 'dW', 'dW_compressed', 'W_old', 'A', 'local_iteration', 'dataset',
                    ...
                    'client_config', 'compression_config', 'device']
        res_state = {}
    ...
    def compress_weight(self, compression_config=None):
        ...
        if accumulate:
            ...
            tl.compress(target=self.dW_compressed, source=self.A,
                        compress_fun=comp.compression_function(compression_config, self.device))
    ...
    def synchronize_with_server(self, GLOBAL_INFO):
        ...
        self.E = GLOBAL_INFO[0]['E']
        self.start_flag = GLOBAL_INFO[0]['start_flag']
        self.Ht = GLOBAL_INFO[0]['Ht']
...
def get_client_from_temp(client_temp):
    device = client_temp.device
    client = ERAFLClient(cid=client_temp.cid,
...
def run_client(client_temp, STOP_EVENT, SELECTED_EVENT, GLOBAL_QUEUE, GLOBAL_INFO):
    ...
    while not STOP_EVENT.value:
        ...
        if SELECTED_EVENT[cid]:
            ...
            # Estimate staleness s, line 1
            delta_ti = beta = client.size_of_weight / client.bandwidth
            s = delta_ti // client.Ht
            # Local iteration, line 2
            client.local_iteration = client.E_init
            # start flag, line 3,4,5
            if client.start_flag:
                E_i = client.E * s
                client.local_iteration = E_i
            ...
            print(f'Client {client.cid}, local train takes {before_transmit_time-iter_start:.2f}s')
            # compress gradient
            ...
            print(f'Client {client.cid}, transmit takes {time.time()-before_transmit_time:.2f}s beta={beta}')
            print(f"Client {client.cid}, iter takes {(time.time()-iter_start):.2f}s")
    print("Client {} Exit.\n".format(client.cid))
