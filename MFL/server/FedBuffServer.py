...
from ..client.FedBuffClient import run_client
...
class FedBuffServer:
    ...
    def start(self, STOP_EVENT, SELECTED_EVENT, GLOBAL_QUEUE, GLOBAL_INFO):  # start the whole training priod
        ...
        last=time.time()
        interval=0.5 #check buff every 0.5 second
    ...
    def update(self,STOP_EVENT, SELECTED_EVENT, GLOBAL_QUEUE,GLOBAL_INFO):
        ...
        if self.current_epoch >= self.total_epoch:  # if global training is going on
            return
        no_op=True
        K = self.global_config["updater"]["params"]["K"]
        if GLOBAL_QUEUE.qsize() >= K:   # if server has received some gradients from clients
            no_op=False
