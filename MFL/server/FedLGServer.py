...
from ..client.FedLGClient import run_client
...
class FedLGServer:
    def __init__(self, global_config, dataset, compressor_config, clients, device):
        ...
        # alpha and beta of clients
        self.alphas = [1 for i in range(global_config["n_clients"])]
        self.betas = [1 for i in range(global_config["n_clients"])]
        self.ks = [1 for i in range(global_config["n_clients"])]
        self.deltas = [1 for i in range(global_config["n_clients"])]
    ...
    def update(self,STOP_EVENT, SELECTED_EVENT, GLOBAL_QUEUE,GLOBAL_INFO):
        ...
        if not GLOBAL_QUEUE.empty():            # if server has received some gradients from clients
            ...
            tl.weighted_average(target=self.dW,
                                ...
                                weights=torch.Tensor([1 for _ in range(len(data_nums))]))             # global gradient
    ...
    def initialize(self, GLOBAL_INFO):
        # test alpha and beta
        GLOBAL_INFO.append([0, 0])          # the first is alpha and the second is beta
        for client in self.global_manager.clients_list:
            client.testAlphaBeta(GLOBAL_INFO)
            alpha = self.alphas[client.cid] = GLOBAL_INFO[1][0]
            beta = self.betas[client.cid] = GLOBAL_INFO[1][1]

            print("ID: {}, alpha: {}, beta: {}".format(client.cid, alpha, beta))

            def equation1(x, y):
                return (1 / p**2) * ((2 * alpha**2 / y) - (2 * beta**2 * y / x**2) - alpha**2 + (beta**2 * y**2 / x**2)) - (1 / (x**2 * y))

            def equation2(x, y):
                return (1 / p**2) * ((-2 * alpha**2 * x / y**2) - (4 * alpha / y**2) + (2 * beta**2 / x) - 2 * alpha * beta - (2 * beta**2 * y / x)) - (1 / (x * y**2))

            # origin
            # def equation1(x, y):
            #     return (2 * alpha**2 / y) - (2 * beta**2 * y / x**2) - alpha**2 + (beta**2 * y**2 / x**2) - (1 / (x**2 * y))
            
            # def equation2(x, y):
            #     return (-2 * alpha**2 * x / y**2) - (4 * alpha / y**2) + (2 * beta**2 / x) - 2 * alpha * beta - (2 * beta**2 * y / x) - (1 / (x * y**2))

            # get the optimal k and delta
            initial_guess = [1.0, 1.0] 
            p = self.global_config["epoch_time"]
            solutions = fsolve(lambda xy: [equation1(xy[0], xy[1]), equation2(xy[0], xy[1])], initial_guess)
            
            x_solution, y_solution = solutions

            print(f"ID: {client.cid}, K: {x_solution}, Delta: {y_solution}")
        
            self.ks[client.cid] = x_solution
            self.deltas[client.cid] = y_solution
            GLOBAL_INFO[1][0] = x_solution
            GLOBAL_INFO[1][1] = y_solution
            client.receiveKDelta(GLOBAL_INFO)
    ...
    def save_model(self):
        if len(self.loss_list) == 0:
            return
        dir_name=dir_name="{}_{}_{}_{}_FedLG".format(
                            self.global_config["model"], self.global_config["dataset"],
                            self.global_config["local iteration"],self.compressor_config["uplink"]["params"]["cr"])
